...
global.fetch = jest.fn((url) => {
  if (url === `${global.config.urlBack}/voiture/get/all/${0}/${global.config.ligneParPage}`) {
    return Promise.resolve({
      ok: true,  // Simulate a successful response
      json: () => Promise.resolve({
        "volume": 102,
        "voitures": [ "{\"id\":26238,\"marque\":\"Panzerkampfwagen VI Tiger II\",\"modele\":\"Sd Kfz 182\",\"finition\":\"700 ch 860 litres Canon 88 mm\",\"carburant\":\"ESSENCE\",\"km\":10000,\"annee\":1942,\"prix\":1000000}", "{\"id\":26239,\"marque\":\"Panzerkampfwagen VI Tiger II\",\"modele\":\"Sd Kfz 182\",\"finition\":\"700 ch 860 litres Canon 88 mm\",\"carburant\":\"ESSENCE\",\"km\":10000,\"annee\":1942,\"prix\":1000000}", "{\"id\":26241,\"marque\":\"Panzerkampfwagen VI Tiger II\",\"modele\":\"Sd Kfz 182\",\"finition\":\"700 ch 860 litres Canon 88 mm\",\"carburant\":\"ESSENCE\",\"km\":10000,\"annee\":1942,\"prix\":1000000}", "{\"id\":26242,\"marque\":\"Panzerkampfwagen VI Tiger II\",\"modele\":\"Sd Kfz 182\",\"finition\":\"700 ch 860 litres Canon 88 mm\",\"carburant\":\"ESSENCE\",\"km\":10000,\"annee\":1942,\"prix\":1000000}", "{\"id\":26243,\"marque\":\"Panzerkampfwagen VI Tiger II\",\"modele\":\"Sd Kfz 182\",\"finition\":\"700 ch 860 litres Canon 88 mm\",\"carburant\":\"ESSENCE\",\"km\":10000,\"annee\":1942,\"prix\":1000000}" ]
      }),
    });
  } else {
...
const { afficherTableauListeVoitures, 
        effacerFormulaire, 
        ...
        dispatchContexte,
        rechercher,
        accueil } = require('./esieaFront/src/main/webapp/stockcar.js'); 
...
describe('Test afficherTableauListeVoitures', () => {
  test('fetches voiture data from the API', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('http://example.com/api/voiture/get/all/0/5');
    await setTimeout(() => {
...
describe('Test displaying elements', () => {
  test('should display and hide the correct elements', () => {
    ...
    expect(document.querySelector('div.pagination').style.display).toBe("block");
  });
});
  ...
  beforeEach(() => {
    ...
    global.fetch = jest.fn((url) => {
      if (url === `${global.config.urlBack}/voiture/get/1`) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ voiture: { id: 1, marque: 'Citroen' } })
        });
      } else {
...
describe('Test genererEntetesListeVoitures', () => {
  test('should generate table headers correctly', () => {
    ...
    const expectedHeaders = ['Marque', 'Modèle', 'Finition', 'Carburant', 'Kilométrage', 'Année', 'Prix', ' '];
...
describe('Test ajouterContenuListeVoitures', () => {
  test('should add car data to the table correctly', () => {
    ...
    const mockData = {
      voitures: [
        JSON.stringify({ id: 1, marque: 'Citroen', modele: 'C3', finition: 'Blanche', carburant: 'Disesel', km: 50000, annee: 2018, prix: 15000 }),
        JSON.stringify({ id: 2, marque: 'Renault', modele: 'Twingo', finition: 'Clim', carburant: 'Essence', km: 30000, annee: 2019, prix: 18000 })
      ]
    };
    ...
    const expectedValues = [
      ['Citroen', 'C3', 'Blanche', 'Disesel', '50000', '2018', '15000 €', 'Détails'],
      ['Renault', 'Twingo', 'Clim', 'Essence', '30000', '2019', '18000 €', 'Détails']
    ];
    ...
    rows.forEach((row, rowIndex) => {
      ...
      cells.forEach((cell, cellIndex) => {
        if (cellIndex === cells.length - 1) {
          ...
          expect(link.getAttribute('onclick')).toBe(`afficherVoiture(${rowIndex + 1})`);
        } else {
...
describe('selectionnerPage', () => {
  ...
  test('should select the first page and disable the previous button', () => {
    ...
    expect(document.querySelector('div.pagination a.previous').classList.contains('disabled')).toBe(true);
    expect(document.querySelector('div.pagination a.previous').hasAttribute('href')).toBe(false);
  });
  ...
  test('should select a middle page and enable the previous and next buttons', () => {
    ...
    expect(document.querySelector('div.pagination a.previous').classList.contains('disabled')).toBe(false);
    expect(document.querySelector('div.pagination a.previous').getAttribute('href')).toBe('#');
    expect(document.querySelector('div.pagination a.previous').getAttribute('onclick')).toBe('paginer(2)');

    expect(document.querySelector('div.pagination a.next').classList.contains('disabled')).toBe(false);
    expect(document.querySelector('div.pagination a.next').getAttribute('href')).toBe('#');
    expect(document.querySelector('div.pagination a.next').getAttribute('onclick')).toBe('paginer(4)');
  });
  ...
  test('should select the last page and disable the next button', () => {
    ...
    expect(document.querySelector('div.pagination a.next').classList.contains('disabled')).toBe(true);
    expect(document.querySelector('div.pagination a.next').hasAttribute('href')).toBe(false);
  });
});
